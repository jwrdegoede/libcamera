...
int PipelineHandlerCamss::configure(Camera *camera, CameraConfiguration *c)
{
	...
	CamssCameraData *data = cameraData(camera);
	V4L2DeviceFormat csiFormat, outputFormat;
	int ret;
	...
	ret = data->isp_->configure(config->ispConfig_, &csiFormat, &outputFormat);
	if (ret)
		return ret;

	return validateConfigMatchesV4L2DeviceFormat(config->ispConfig_, outputFormat);
}
