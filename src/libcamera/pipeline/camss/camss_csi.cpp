 ...
 * \param[in] useIspMargins Take ISP debayering margin into account?
 *
 ...
 */
V4L2SubdeviceFormat CamssCsiCamera::getSensorFormat(Size size,
						    const PixelFormat &format,
						    bool useIspMargins) const
{
	...
	for (unsigned int code : mbusCodes) {
		...
		if (useIspMargins)
			margin = isp_->getMargins(pixelFormat);

		for (const Size &sz : sizes) {
			if (sz.width < (size.width + margin.width) ||
			    sz.height < (size.height + margin.height))
				continue;
			...
			unsigned int area = (sz.width - margin.width) *
					    (sz.height - margin.height);
...
int CamssCsiCamera::tryFormat(const V4L2SubdeviceFormat &sensorFormat,
			      StreamConfiguration &cfg) const
{
...
StreamConfiguration CamssCsiCamera::validate(const StreamConfiguration &req) const
{
	...
	V4L2SubdeviceFormat sensorFormat = getSensorFormat(req.size, req.pixelFormat, true);
...
int CamssCsiCamera::configure(const StreamConfiguration &cfg, const Transform &transform)
{
	...
	sensorFormat = getSensorFormat(cfg.size, cfg.pixelFormat, false);
	...
	if (!camssV4L2DeviceFormatMatchesStreamConfig(outputFormat, cfg, "CSI configure"))
		return -EIO; /* Must match the tryFormat() from validate() */
