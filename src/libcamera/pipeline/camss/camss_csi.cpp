 ...
 * media bus codes with the formats supported by the CamssCsi unit.
 *
 * \return The list of supported PixelFormat
 */
std::vector<PixelFormat> CamssCsiDevice::formats() const
{
	if (!sensor_)
		return {};
 ...
 * the sensor produced media bus codes formats supported by the CamssCsi unit.
 *
 * \return A list of supported sizes for the \a format or an empty list
 * otherwise
 */
std::vector<SizeRange> CamssCsiDevice::sizes(const PixelFormat &format) const
{
	int mbusCode = -1;
...
/**
 * \brief Initialize components of the CamssCsi device with \a index
 * \param[in] media The CamssCsi media device
 * \param[in] index The CamssCsi device index
 *
 * Create and open the video device and subdevices in the CamssCsi instance at \a
 * index, if a supported image sensor is connected to the CSI-2 receiver of
 * this CamssCsi instance.  Enable the media links connecting the CamssCsi components
 * to prepare for capture operations and cached the sensor maximum size.
 *
 * \return 0 on success or a negative error code otherwise
 * \retval -ENODEV No supported image sensor is connected to this CamssCsi instance
 */
int CamssCsiDevice::init(std::shared_ptr<const MediaDevice> media, unsigned int index)
{
	int ret;

	/*
	 * Verify that a sensor subdevice is connected to this CamssCsi instance
 ...
 * \param[out] outputFormat The CamssCsi unit output image format
 * \return 0 on success or a negative error code otherwise
 */
int CamssCsiDevice::configure(const Size &size, const Transform &transform,
			  V4L2DeviceFormat *outputFormat)
{
	...
	LOG(IPU3, Debug) << "CamssCsi output format " << *outputFormat;

	return 0;
}

StreamConfiguration CamssCsiDevice::generateConfiguration(Size size) const
{
 ...
 */
V4L2SubdeviceFormat CamssCsiDevice::getSensorFormat(const std::vector<unsigned int> &mbusCodes,
...
int CamssCsiDevice::exportBuffers(unsigned int count,
			      std::vector<std::unique_ptr<FrameBuffer>> *buffers)
{
	return output_->exportBuffers(count, buffers);
}

int CamssCsiDevice::start()
{
	...
	if (ret)
		LOG(IPU3, Error) << "Failed to import CamssCsi buffers";
...
FrameBuffer *CamssCsiDevice::queueBuffer(Request *request, FrameBuffer *rawBuffer)
{
	...
	if (!buffer) {
		if (availableBuffers_.empty()) {
			LOG(IPU3, Debug) << "CamssCsi buffer underrun";
			return nullptr;
		}
...
void CamssCsiDevice::tryReturnBuffer(FrameBuffer *buffer)
{
	/*
...
void CamssCsiDevice::freeBuffers()
{
	availableBuffers_ = {};
	buffers_.clear();

	if (output_->releaseBuffers())
		LOG(IPU3, Error) << "Failed to release CamssCsi buffers";
}
