	...
	: opeMediaDev_(std::move(opeMediaDev)), sensor_(sensor), frameInfos_(frameInfos)
{
	...
	ipa_ = pipe->createIPA<ipa::softisp::IPAProxySoftIsp>("simple", 0, 0);
	if (!ipa_) {
...
void CamssIspOpe::paramsComputed(uint32_t frame)
{
	...
	awbGains->r_gain = std::clamp(debayerParams.gains.r(), 0.0, 31.0) * 1024;
	awbGains->g_gain = std::clamp(debayerParams.gains.g(), 0.0, 31.0) * 1024;
	awbGains->b_gain = std::clamp(debayerParams.gains.b(), 0.0, 31.0) * 1024;
