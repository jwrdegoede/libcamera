...
int CamssIspOpe::trySetVideoCfg(V4L2VideoDevice *v4l2Dev, const StreamConfiguration &cfg,
...
StreamConfiguration CamssIspOpe::generateConfiguration(const StreamConfiguration &raw) const
{
	...
	if (trySetVideoCfg(input_.get(), raw, false, "OPE input try format"))
		return {};
				    ...
				    const char *msgPrefix) const
{
	...
	if (subdevFormat.size != fmt.size) {
		LOG(Camss, Error) << msgPrefix << " size mismatch asked " << fmt.size
				  << " got " << subdevFormat.size;
		return -EINVAL;
	}
				      ...
				      V4L2Subdevice::Whence whence) const
{
	 ...
	 * \todo crop in x or y dimension to match requested output aspect, to
	 * keep square pixels after scaling.
	 */
	Rectangle inputCrop(inputCfg.size);
	/*
	 * FIXME this needs a trySetSelection helper which checks we got what we asked,
	 * like trySetSubdevFormat().
	 */
	ret = proc_->setSelection(PROC_PAD_INPUT, V4L2_SEL_TGT_CROP, &inputCrop, whence);
	if (ret < 0) {
		LOG(Camss, Error) << "OPE try/set input crop error: " << strerror(-ret);
		return ret;
	}

	Rectangle outputCompose(outputCfg.size);
	ret = proc_->setSelection(PROC_PAD_DISP, V4L2_SEL_TGT_COMPOSE, &outputCompose, whence);
	if (ret < 0) {
		LOG(Camss, Error) << "OPE try/set output compose error: " << strerror(-ret);
		return ret;
	}
...
#if 0 /* FIXME this is disabled for now due to kernel driver issues */
			   ...
			   const StreamConfiguration &outputCfg)
{
	...
	ret = trySetVideoCfg(input_.get(), inputCfg, true, "OPE input set format");
	if (ret < 0)
		return ret;
	...
	ret = trySetVideoCfg(output_.get(), outputCfg, true, "OPE output set format");
	if (ret < 0)
		return ret;
