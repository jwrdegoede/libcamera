	...
	: sensor_(sensor), frameInfos_(frameInfos)
{
	...
	swIsp_->ispStatsReady.connect(this, &CamssIspSoft::statsReady);
...
void CamssIspSoft::statsReady(uint32_t frame, uint32_t statsBufferId)
{
	CamssFrames::Info *info = frameInfos_->find(frame);
	if (!info) {
		LOG(Camss, Error) << "Cannot find frame info for frame " << frame;
		return;
	}

	swIsp_->processStats(frame, statsBufferId, info->effectiveSensorControls);
}
