...
int SimpleCameraData::init()
{
	...
	/*
	 * Create Linaro's implementation of SoftwareIsp unconditionally if no converter is used
	 * - to be revisited
	 */
	if (!converter_) {
		swIsp_ = std::make_unique<SwIspLinaro>("linaro");
		if (!swIsp_) {
			LOG(SimplePipeline, Warning)
		...
		/* Create Linaro's IPA for the SoftwareIsp - to be revisited */
		ipa_ = IPAManager::createIPA<ipa::soft::IPAProxySoft>(pipe, 0, 0);
		if (!ipa_) {
			LOG(SimplePipeline, Warning) << "Creating IPA for software ISP failed";
			return -ENOENT;
		}
		ret = ipa_->init(IPASettings{ "No cfg file", "No sensor model" },
				 ...
				 sensor_->controls());
		if (ret) {
			LOG(SimplePipeline, Warning) << "IPA init failed";
			return ret;
		}
...
void SimpleCameraData::bufferReady(FrameBuffer *buffer)
{
	...
	if (buffer->metadata().status != FrameMetadata::FrameSuccess) {
		...
		/*
		 * The converter or Software ISP is in use. Requeue the internal buffer for
	...
	/*
	 * Queue the captured and the request buffer to the converter or Software ISP
	 * if format conversion is needed. If there's no queued request, just requeue
	 * the captured buffer for capture.
	 */
	if (useConversion_) {
		if (conversionQueue_.empty()) {
			video_->queueBuffer(buffer);
